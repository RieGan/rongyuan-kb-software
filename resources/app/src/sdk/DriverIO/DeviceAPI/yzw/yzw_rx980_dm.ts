...
import { defaultMatrix_yzw_rx980_dm } from "./yzw_rx980_dmMatrix"

export class YZW_RX980_DM extends YZW_KC68 {
    defaultMatrix = defaultMatrix_yzw_rx980_dm
    setLightSetting = async (lightSet: LightSetting) => {
        ...
        switch (lightSet.type) {
            ...
            case 'LightWave':
                effect = 0x04
                speed = lightSet.speed
                option = lightSet.option !== undefined ? (this.WAVEOP[lightSet.option] << 4 | n) : (this.WAVEOP['down'] << 4 | n)
                brightness = lightSet.value
                break
            case 'LightRipple':
                effect = 0x05
