...
import { defaultMatrix_yzw_gzk61 } from "./yzw_gzk61Matrix"
...
export class YZW_GZK61_2M_24 extends YZW_K68 {
    defaultMatrix = defaultMatrix_yzw_gzk61
    ...
    setLightSetting = async (lightSet: LightSetting) => {
        ...
        switch (lightSet.type) {
            ...
            case 'LightWave':
                effect = 0x04
                speed = lightSet.speed
                option = lightSet.option !== undefined ? (waveOp[lightSet.option] << 4 | this.DAZZLE) : (waveOp['right'] << 4 | this.DAZZLE)
                brightness = lightSet.value
                break
            case 'LightRipple':
                effect = 0x05
