...
import { defaultMatrix_yzw_sk1 } from "./yzw_sk1Matrix"
...
export class YZW_SK1 extends YZW_K68 {
    defaultMatrix = defaultMatrix_yzw_sk1
    ...
    getLightSetting = async (): Promise<LightSetting | undefined> => {
        ...
        const optionToRgb = (op: number) => {
            if (effect === 0x11 || effect === 0x13) return
        ...
        switch (effect) {
            ...
            case 0x0d:
                const uOp = valueForKey(this.USEROP, option1 << 4)
                return {
