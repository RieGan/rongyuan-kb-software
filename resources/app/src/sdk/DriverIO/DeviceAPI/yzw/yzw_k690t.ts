...
import { defaultMatrix_yzw_k690t } from "./yzw_k690tMatrix";
import { YZW_KC68 } from "./yzw_kc68";
...
export class YZW_K690T extends YZW_KC68 {
    defaultMatrix = defaultMatrix_yzw_k690t
    DAZZLE = 8;
    NORMAL = 9;
    ...
    setLightSetting = async (lightSet: LightSetting) => {
        ...
        switch (lightSet.type) {
            ...
            case 'LightWave':
                effect = 0x04
                speed = lightSet.speed
                option = lightSet.option !== undefined ? (waveOp[lightSet.option] << 4 | n) : (waveOp['down'] << 4 | n)
                brightness = lightSet.value
                break
            case 'LightRipple':
                effect = 0x05
            ...
            case 'LightMusicFollow2':
                effect = 0x16
                brightness = lightSet.value
                option = lightSet.option === undefined || lightSet.option === 'upright' ? (0 << 4) | musicKey : mp[lightSet.option] << 4 | musicKey
                break
            case 'LightTrain':
                effect = 0x17
                speed = lightSet.speed
    ...
    getLightSetting = async (): Promise<LightSetting | undefined> => {
        ...
        switch (effect) {
            ...
            case 0x16:
                const op2 = valueForKey(mp, option1)
                return {
