...
const TextInputStyled = styled.div(
  (p: { type: keyof typeof TypeTable | undefined }) => ({
    ...
    '& input': {
      ...
      outline: 'none',
      paddingLeft: '15px',
      border: TypeTable[p.type || 'Default_akko'].border,
      background: TypeTable[p.type || 'Default_akko'].background,
      '::-webkit-input-placeholder': {
        textAlign: TypeTable[p.type || 'Default_akko'].textAlgin,
      },
      borderRadius: TypeTable[p.type || 'Default_akko'].borderRadius,
      textAlign: TypeTable[p.type || 'Default_akko'].textAlgin,
...
export const TextInput_akko = (p: {
  ...
  return (
    <TextInputStyled type={p.type}>
      <input
        type={p.type === '密码_akko' ? 'password' : 'text'}
